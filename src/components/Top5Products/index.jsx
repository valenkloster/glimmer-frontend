...
const TopProducts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('Productos recibidos:', topProducts); // Añade este console.log
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto p-3 bg-nude bg-opacity-60 rounded-lg">
      <form onSubmit={handleSubmit} className="w-full max-w-md mb-6 flex flex-wrap gap-3 justify-center">
        ...
        <button
          type="submit"
          className="bg-verde-agua text-white px-3 py-1.5 rounded text-sm hover:opacity-90"
          disabled={loading}
        >
      ...
      {products.length > 0 ? (
        <ul className="w-full bg-white shadow-md rounded-lg p-3">
          ...
          <li key={product.productId} 
              className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 border-b py-4 hover:bg-gray-50 px-3 rounded-lg">
            <span className="text-xl font-bold text-verde-agua w-6">{index + 1}</span>
            <img 
              src={product.image} 
              alt={product.name}
              className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-lg shadow-md hover:scale-105 transition-transform"
            />
